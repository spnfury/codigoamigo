...
function mostrarRecargarSaldo() {
    ...
    if (typeof $ !== 'undefined') {
        try {
            ...
            // Fallback: mostrar modal usando vanilla JavaScript
            mostrarModalVanilla();
        }
    } else {
        mostrarModalVanilla();
...
function destacarTodosCodigos() {
    ...
    stripe.redirectToCheckout({
        ...
        successUrl: '<?php echo $GLOBALS["website"];?>felicidades_destacar_todos?session_id={CHECKOUT_SESSION_ID}',
...
window.filterByVisibility = function(visibility) {
    ...
    codeItems.forEach(function(item) {
        ...
        if (visibility === 'all') {
            ...
            shouldShow = true;
        } else if (visibility === 'destacados' && itemDestacado === 'si') {
            shouldShow = true;
        } else if (visibility === 'no_destacados' && itemDestacado === 'no') {
            shouldShow = true;
        }
