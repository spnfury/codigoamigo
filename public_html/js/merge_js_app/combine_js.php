...
//http://www.ejeliot.com/blog/72
  /****************** start of config ******************/
   ...
   define('CACHE_LENGTH', 9999999999800); // length of time to cache output file, default approx 1 year
   define('CREATE_ARCHIVE', true); // set to false to suppress writing of code archive, files will be merged on each request
   define('ARCHIVE_FOLDER', 'js/merge_js_app'); // location to store archive, don't add starting or trailing slashes
   ...
   */
   ...
   if (isset($_GET['version'])) {
      ...
      if (CREATE_ARCHIVE && file_exists("$sDocRoot/".ARCHIVE_FOLDER."/site_$iETag.js")) {

         $sCode = file_get_contents("$sDocRoot/".ARCHIVE_FOLDER."/site_$iETag.js");
      } else {
         ...
         if (CREATE_ARCHIVE) {
            ...
            if ($iETag == $aLastModifieds[0]) { // check for valid etag, we don't want invalid requests to fill up archive folder
                ...
                $postdata = array('http' => array(
                    ...
                    'content' => http_build_query( array('input' => $sCode) ) ) );
                
                $sCode = file_get_contents('https://javascript-minifier.com/raw', false, stream_context_create($postdata));
            	
               $oFile = fopen("$sDocRoot/".ARCHIVE_FOLDER."/site_$iETag.js", 'w');
               fwrite($oFile, $sCode);                 
               fclose($oFile);
