...
function debug_log($message, $data = null)
{
    ...
    // Usar el nuevo sistema de logging para debug
    if (defined('DEBUG_MODE') && DEBUG_MODE) {
        log_debug($message, ['data' => $data]);
    } else {
        // Solo escribir al archivo de debug en modo desarrollo
        error_log($log . "\n", 3, __DIR__ . '/debug.log');
    }
}
...
if (filter_input(INPUT_GET, "continua_viendo", FILTER_SANITIZE_STRING) == 1) {
...
include_once dirname(__DIR__) . '/myphp/funciones.php';
include_once dirname(__DIR__) . '/myphp/_header.php';
...
$app->get('/ofertas/{termino}', function ($request, $respon, $args) {
    ...
    $from_trends = filter_input(INPUT_GET, "from", FILTER_SANITIZE_STRING) === 'trends';
    if (!$from_trends) {
        try { 
...
$app->get('/', function ($request, $respon) {
    ...
    $lista_codigos = array_merge(
        (is_array($lista_codigos_patrocinados) && !empty($lista_codigos_patrocinados) ? array_slice($lista_codigos_patrocinados, 0, 3) : []),
...
$app->get('/bienvenido_de_nuevo', function ($request, $respon) {
    ...
    //     /* Confirmación del mail del usuario y activación para que pueda logearse en la web */
//     $title = "Bienvenido a Código Amigo";
//     $description = "La única web donde puedes compartir tus códigos - " . $GLOBALS["author"];
...
//     /* Cambiamos estado del usuario (1 activado / 0 desactivado) para que el usuario pueda iniciar sesión */
//     activeUserToLogin($cadena_desencriptada);
    ...
    //     $mail_encriptado = filter_input(INPUT_GET, "codigo", FILTER_SANITIZE_STRING);
    //     $cadena_encriptada_correcta = str_replace(" ", "+", $mail_encriptado);
    $user_correcto = str_replace(" ", "+", filter_input(INPUT_GET, "user", FILTER_SANITIZE_STRING));
    $codigo_recibido = filter_input(INPUT_GET, "codigo", FILTER_SANITIZE_STRING);

    // Desencriptar el código recibido y comparar con el user
    $user_desencriptado = desencriptar($codigo_recibido);

    if ($user_desencriptado == $user_correcto) {
        activeUserToLogin($user_correcto);
    } else {
        echo "error en el código de activación";
        die;
    }

    include_once $_SERVER['DOCUMENT_ROOT'] . '/public/bienvenido_de_nuevo.php';
